...
if [ -n "${PENGPILOT_CODESIGN_IDENTITY:-}" ]; then
  codesign_identity="$PENGPILOT_CODESIGN_IDENTITY"
else
...
case "$profile" in
  debug)
    app_name="PengPilot Debug"
    helper_name="PengPilot Debug Computer Use"
    bundle_identifier="com.pengpilot.app.debug"
    icon_file="AppIconDev.icns"
    ;;
  release)
    app_name="PengPilot"
    helper_name="PengPilot Computer Use"
    bundle_identifier="com.pengpilot.app"
...
repl_executable="$contents/Resources/pengpilot_js_repl"
daemon_executable="$contents/MacOS/pengpilot-daemon"
...
helper_cache_root=".pengpilot-cache/computer-use/$profile"
...
# The cached app is copied into PengPilot's standard Helpers directory as the
# canonical packaged service. PengPilot refreshes a stable standalone runtime
# copy from it so Screen Recording is attributed to the helper, not PengPilot.
...
if [ ! -d "$cached_helper_bundle" ]; then
  ...
  printf '%s\n' "$helper_fingerprint" > "$cached_helper_contents/Resources/.pengpilot-helper-fingerprint"
...
sparkle_cache_root=".pengpilot-cache/sparkle"
...
rm -rf "$bundle"
mkdir -p "$contents/MacOS" "$contents/Resources/computer-use" "$contents/Resources/skills/pengpilot-computer-use" "$contents/Helpers"
cp "$cargo_target_dir/$profile/pengpilot" "$contents/MacOS/$app_name"
cp "$cargo_target_dir/$profile/pengpilot_js_repl" "$repl_executable"
chmod 755 "$repl_executable"
if [ "$profile" = "release" ]; then
  cp "$cargo_target_dir/$profile/pengpilot-daemon" "$daemon_executable"
...
cp resources/computer-use/SKILL.md "$contents/Resources/skills/pengpilot-computer-use/SKILL.md"
...
# PengPilot is not sandboxed, so Sparkle's XPC services never run; drop them along
