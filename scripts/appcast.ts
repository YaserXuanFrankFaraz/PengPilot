...
//
// <updates-dir> holds the packaged archives (e.g. PengPilot-0.2.0.zip) plus any
...
//   PENGPILOT_DOWNLOAD_URL_PREFIX   base URL for enclosure links
import { $ } from "bun";
...
export function findGenerateAppcast(): string | null {
  ...
  const cacheRoot = join(projectRoot, ".pengpilot-cache", "sparkle");
  ...
  if (!generator) {
    throw new Error(
      "generate_appcast not found. Run scripts/bundle.sh once to populate " +
        ".pengpilot-cache/sparkle, or set SPARKLE_BIN to a Sparkle tools bin/ dir.",
    );
  }
  console.log(`Using: ${generator}`);
  // Same prefix for both: archives and PengPilot-<version>.md release notes are
