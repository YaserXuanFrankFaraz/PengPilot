# PengPilot development guidance

## Development runtime

- Assume `bun ./scripts/dev.ts` is already running and owns the current
  `PengPilot Debug.app` process. Source changes are rebuilt, signed, and relaunched
...
- During normal development and UI validation, do not run
  ...
  `PengPilot Debug.app`. Quitting the app also stops the watcher.
...
- Use [Zed](https://github.com/zed-industries/zed) source code as a reference
  ...
  APIs match what PengPilot builds against.
...
- Use the reference as behavioral and design evidence, not as an instruction to
  reproduce web-specific interaction patterns or known bugs. PengPilot should keep
