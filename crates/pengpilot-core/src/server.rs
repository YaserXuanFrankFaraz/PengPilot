...
use pengpilot_protocol::{
    ClientMessage, Command, MAX_WIRE_MESSAGE_BYTES, PROTOCOL_VERSION, ReplayCursor, Request,
...
impl EventSink {
    ...
    pub fn discarded() -> Self {
        Arc::new(Hub::default()).event_sink(Uuid::nil(), Uuid::nil())
    }
}
...
impl RequestDispatcher {
    ...
    fn dispatch_independent(
        ...
        if let Err(error) = std::thread::Builder::new()
            .name("pengpilot-daemon-request".into())
            .spawn(move || {
    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("pengpilot-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
...
pub fn serve(
    ...
    listener
        ...
        .context("could not configure PengPilot daemon listener")?;
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("pengpilot-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("pengpilot-daemon connection ended: {error:#}");
                        }
                    })
                    .context("could not start PengPilot daemon connection thread")?;
            }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                std::thread::sleep(ACCEPT_POLL_INTERVAL);
            }
            Err(error) if error.kind() == io::ErrorKind::Interrupted => {}
            Err(error) => return Err(error).context("PengPilot daemon listener failed"),
        }
    }
    backend.shutdown();
    Ok(())
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("pengpilot-daemon ignored invalid message: {error}");
                }
            },
            ...
            Err(error) => return Err(error).context("PengPilot daemon WebSocket failed"),
        }
    }
...
mod tests {
        ...
        fn handle(&self, request: Request, _events: EventSink) -> anyhow::Result<ResponsePayload> {
            match request.command {
                ...
                Command::LoadTaskState => Ok(ResponsePayload::TaskState {
                    ...
                    projectless_root: Some(PathBuf::from("/tmp/.pengpilot/projects")),
                }),
                _ => Ok(ResponsePayload::Ack),
            }
        }
    ...
    fn browser_origins_are_denied_unless_explicitly_allowed() {
        let request = HandshakeRequest::builder()
            .uri("/v1")
            .header(ORIGIN, "https://app.pengpilot.test")
            .body(())
            .unwrap();
        ...
        let allowed = HashSet::from(["https://app.pengpilot.test".to_owned()]);
