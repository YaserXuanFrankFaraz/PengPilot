...
pub use pengpilot_protocol::workspace::{
...
pub fn execute(operation: WorkspaceOperation) -> anyhow::Result<WorkspaceResult> {
    Ok(match operation {
        ...
        WorkspaceOperation::MigrateProjectlessWorkspace { path } => {
            if !path.is_dir() {
                bail!(
                    "projectless workspace is not a directory: {}",
                    path.display()
                );
            }
            WorkspaceResult::ProjectlessWorkspace { cwd: path }
        }
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("pengpilot-workspace-{}", Uuid::new_v4()));
        ...
        git_ok(
            ...
            &[
                "-c",
                "user.name=PengPilot Tests",
                "-c",
                "user.email=pengpilot@example.com",
                "commit",
                "-m",
                "baseline",
            ],
    ...
    fn directory_browser_lists_an_arbitrary_daemon_directory() {
        let directory =
            std::env::temp_dir().join(format!("pengpilot-directory-browser-{}", Uuid::new_v4()));
    ...
    fn file_reader_keeps_the_complete_disk_content() {
        let root = std::env::temp_dir().join(format!("pengpilot-editor-file-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        let content = "line\n".repeat(10_000);
