...
pub use pengpilot_protocol::model::{
    FavoriteModel, InteractionMode, ProviderAgentPreset, ProviderKind,
    ProviderModel, ProviderModelOption, ProviderResumeCursor, RuntimeMode,
};
...
/// What the provider said a file change did to its file. `None` when the
/// event did not carry a change type.
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
