...
pub use pengpilot_protocol::theme::ThemePreference;

fn preference_resolves_to_dark(
...
fn preference_native_override(preference: ThemePreference) -> Option<bool> {
    match preference {
...
pub fn init(cx: &mut App) {
    ...
    let theme = if preference_resolves_to_dark(ThemePreference::System, system_appearance) {
        Theme::dark()
    } else {
        Theme::light()
    };
...
pub fn apply_theme_preference(preference: ThemePreference, window: &mut Window, cx: &mut App) {
    crate::platform::set_window_appearance(window, preference_native_override(preference));
    let is_dark = preference_resolves_to_dark(preference, cx.window_appearance());
    set_active_theme(
        if is_dark {
            Theme::dark()
        } else {
