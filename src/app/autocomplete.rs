                    ...
                    let commands = cx
                        ...
                        .spawn(async move {
                            match workspace.request(
                                pengpilot_client::WorkspaceOperation::DiscoverSlashCommands {
                                    provider,
                                    project_root: path,
                                },
                            ) {
                                Ok(pengpilot_client::WorkspaceResult::SlashCommands { commands }) => {
                                    commands
                                }
                                Ok(_) | Err(_) => Vec::new(),
                            }
            ...
            Query::Missing(token) => {
                ...
                cx.spawn(async move |waku, cx| {
                    let files = cx
                        ...
                        .spawn(async move {
                            match workspace.request(
                                pengpilot_client::WorkspaceOperation::ListProjectFiles {
                                    root: path,
                                    cap: FILE_INDEX_CAP,
                                },
                            ) {
                                Ok(pengpilot_client::WorkspaceResult::ProjectFiles { entries }) => {
                                    entries
                                }
                                Ok(_) | Err(_) => Vec::new(),
                            }
