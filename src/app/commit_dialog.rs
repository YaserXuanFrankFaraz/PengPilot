    ...
    pub(super) fn open_commit_dialog(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |waku, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    match workspace_client.request(pengpilot_client::WorkspaceOperation::InspectCommit {
                        cwd: workspace.clone(),
                    }) {
                        Ok(pengpilot_client::WorkspaceResult::CommitSnapshot { snapshot }) => {
                            Ok(snapshot)
                        }
            ...
            let result = cx
                ...
                .spawn(async move {
                    match workspace_client.request(
                        pengpilot_client::WorkspaceOperation::GenerateCommitMessage {
                            cwd: generation_workspace,
                            include_unstaged,
                            invocation,
                        },
                    ) {
                        Ok(pengpilot_client::WorkspaceResult::CommitMessage { message }) => {
                            Ok(message)
                        }
                        Ok(_) => Err("the daemon returned an invalid Git response".to_owned()),
                        Err(error) => Err(error.to_string()),
            ...
            let result = cx
                ...
                .spawn(async move {
                    let operation = match action {
                        CommitAction::Commit => pengpilot_client::WorkspaceOperation::Commit {
                            cwd: operation_workspace.clone(),
                            message,
                            include_unstaged,
                            push: false,
                        },
                        CommitAction::CommitAndPush => pengpilot_client::WorkspaceOperation::Commit {
                            cwd: operation_workspace.clone(),
                            message,
                            include_unstaged,
                            push: true,
                        },
                        CommitAction::Push => pengpilot_client::WorkspaceOperation::Push {
                            cwd: operation_workspace.clone(),
                        },
                    };
                    let result = match workspace_client.request(operation) {
                        Ok(pengpilot_client::WorkspaceResult::Ack) => Ok(()),
                    ...
                    let snapshot = result.as_ref().err().and_then(|_| {
                        match workspace_client.request(
                            pengpilot_client::WorkspaceOperation::InspectCommit {
                                cwd: operation_workspace.clone(),
                            },
                        ) {
                            Ok(pengpilot_client::WorkspaceResult::CommitSnapshot { snapshot }) => {
